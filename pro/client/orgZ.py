...
if rank == 0:
    ...
    start_time_zipping = time.time()
    with zipfile.ZipFile("model.zip", "w") as zipf:
        zipf.write("model.pth")
    end_time_zipping = time.time()
    
    tot_time_zipping = (end_time_zipping-start_time_zipping)
        
    with open("model.zip", "rb") as f:
        model_bytes = f.read()
        
    model_tensor = torch.ByteTensor(list(model_bytes))
    size_tensor = torch.tensor([model_tensor.numel()], dtype=torch.long)
    process.send(size_tensor, dst=1)  
    
    start_time = time.time()
    process.send(model_tensor, dst=1)
    end_time = time.time()
    
    tot_time = (end_time - start_time)
    
    logging.info(f"model zipped sent(zipping + sent) time = {tot_time+tot_time_zipping}")
