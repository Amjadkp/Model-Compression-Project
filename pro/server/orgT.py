...
logging.basicConfig(filename="/app/server.log",format='%(asctime)s %(message)s', level=logging.INFO, datefmt='%Y-%m-%d_%H-%M-%S')
...
process.recv_model(model=model,src=0)
end_time = time.time()

tot_time = (end_time - start_time)

logging.info(f"model not zipped recv time = {tot_time}")